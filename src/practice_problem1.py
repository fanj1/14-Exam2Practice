...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        self.volume = volume
        self.int_volume = volume
        if len(contents) <= self.volume:
            self.contents = contents
            self.int_contents = contents
        else:
            self.contents = ''
            self.int_contents = ''
        self.discard = 0
    ...
    def double(self):
        ...
        ans = ''
        con = self.contents
        if len(self.contents) + len(con) > self.volume:
            for k in range(self.volume - len(self.contents), len(con)):
                ans = ans + con[k]
            for k in range(0, self.volume - len(self.contents)):
                self.contents = self.contents + con[k]
        else:
            for k in range(0, len(con)):
                self.contents = self.contents + con[k]
        self.discard = self.discard + len(ans)
        return ans
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > new_volume:
            self.contents = ''
            for k in range(new_volume, len(con)):
                ans = ans + con[k]
    ...
    def reset(self):
        ...
        self.contents = self.int_contents
        self.volume = self.int_volume
    ...
    def steal(self, other_box):
        ...
        con = self.contents
        other_con = other_box.contents
        other_box.contents = ''
        if self.volume - len(self.contents) >= len(other_con):
            self.contents = con + other_con
        else:
            for k in range(self.volume, len(other_con)):
                other_box.contents = other_box.contents + other_con[k]
            for k in range(0, self.volume):
                self.contents = self.contents + other_con[k]
